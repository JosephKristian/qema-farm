      ...
      <div className='flex flex-row justify-start'>
        ...
        <div className='flex-1 flex flex-col items-center space-y-4 px-10 my-[90px] overflow-y-auto'>
          <div className='w-full flex flex-row justify-between items-center'>
            <button onClick={() => showAddModal()} className='bg-[#145412] text-white font-medium text-lg rounded-md px-4 py-2'>Tambah</button>
            <input type='text' defaultValue={search} onChange={(e) => setSearch(e.target.value)} placeholder='Cari Paket' className='w-96 border border-gray-200 rounded-md self-end outline-none p-2' />
          </div>
          <div className='py-6 rounded-lg border border-slate-300 bg-slate-200 shadow-md'>
            <table class="bg-white w-full table-fixed border-collapse bordertext-sm">
              ...
              <tbody>
                {
                  packageRed.filter(e => e.name.toLowerCase().includes(search.toLowerCase().trim())).map(element => {
                    index++;
                    return (
                      <tr key={element.uid} className={(index % 2) === 0 ? 'text-base font-medium bg-gray-100' : 'text-base font-medium'}>
                        ...
                        <td className='border border-slate-200 overflow-auto p-2 text-[#333333] capitalize'>{element.goat.name} {element.goat.sex} ({element.goat.type})</td>
                        ...
                        <td className='border border-slate-200 overflow-auto p-2 text-[#b31818] capitalize'>Rp. <span className={element.discount_price !== 0 ? 'line-through text-[#333333] font-normal' : 'text-[#b31818] text-base font-medium'}>{format.format(element.price).replaceAll(',', '.')}</span> <span className='text-[#b31818] text-base font-semibold'>{element.discount_price !== 0 ? format.format(element.discount_price).replaceAll(',', '.') : ''}</span></td>
                        <td className='flex flex-row space-x-1 overflow-auto p-2'>
                          <button onClick={() => {
                            setPackageSelected(element);
                            setDetailPackageModal(true);
                          }} className='bg-gray-800 rounded-lg text-white px-4 py-2'>Detail</button>
                          <button onClick={() => {
                            setPackageSelected(element);
